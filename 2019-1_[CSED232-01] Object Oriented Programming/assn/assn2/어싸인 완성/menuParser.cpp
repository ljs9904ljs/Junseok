...
public:
	...
	int delVote(string subject) { //중복된 주제로 투표를 만들 수 있지 않나? 항목 수만 다르게 해서. pdf 다시 읽어보기
		int success = 0;
		int i;
...
//메뉴 1번, 중복 검사         *************탈퇴하고 다시 만들때를 고려해야함 수정 필수
...
void login(memberCollection &memList, int loadMode, ifstream & inLogRead, ofstream & inLogWrite, string *tokens, int nTokens)
{
	...
	if (!loadMode) {
		...
		index = findIndex(memList, ID, password); // 멤버리스트에서 index 찾기
